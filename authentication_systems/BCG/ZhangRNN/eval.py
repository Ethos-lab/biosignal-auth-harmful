...
class PairGenerator:
    ...
    def create_examples(self, num_matches=500):
...
def eval(args):
    ...
    orig_dset  = TrainDataset(split='eval', T=T, spoof_name='original',  both_bcg_ecg=args.both)
    spoof_dset = TrainDataset(spoof_name=args.spoof_name, split='eval', T=T, both_bcg_ecg=args.both)  # default T=15 heartbeats

    device = 'cuda:1'

    if 'input_size' in model_dict:
        sample_len = model_dict['input_size']
    else:
        sample_len = orig_dset.SAMPLE_LEN
    model = Model(T=T, num_classes=num_classes, input_size=sample_len)
    ...
    pair_gen = PairGenerator(orig_dset, spoof_dset, baseline=args.baseline)
