...
def load_dataset(data_json, path, in_step, fin_step, parallel):
    ...
    for d in tqdm.tqdm(data):
        ...
        for p in path:
            if os.path.isfile(os.path.join(p, d['ecg'])):
                ...
                found = True
                break
            else: # The ECGID dataset changed I guess and has different filenames than what they hardocded in this repo
                new_d_ecg = os.path.join(os.path.dirname(d['ecg']), 'template_000.csv')
                if os.path.isfile(os.path.join(p, new_d_ecg)):
                    ecg = load_ecg(os.path.join(p, new_d_ecg), in_step, fin_step)
                    found = True
                    break
    ...
    print("Finished load_dataset, found num ecg traces: ", len(data_dict['ecg']))
...
def prepare_dataset(ds_file, params):
    ...
    if extract_features:
        ...
        if lead_i:
            ...
            ds['ecg'] = temp
    elif lead_i and params['experiment'].find('autoencoder') > -1:  # NOTE: Fix added by Veena
        # We dont call utils.feature_extraction here so the data never gets normalized and padded to 12D
        #data_preproc = np.array([(e - preproc_old.mean['ecg']) / preproc_old.std['ecg'] for e in ecgs])
        temp = []
        for arr in ds['ecg']:
            arr2 = np.pad(arr, ((0, 0), (0, 11), (0, 0)))
            temp.append(arr2)
        ds['ecg'] = temp
...
def compute_mean_std(x):
    ...
    try:
        ...
        import math
        size = 20000 * 12  # Why, what is 20000?
        mean_acc = 0
        std_acc = 0
        n_acc = 0
        i = 0
