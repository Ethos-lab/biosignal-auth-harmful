...
def feature_extraction(model_path, output_layer, ecgs):
    ...
    print("Loaded autoencoder: ", model_path)
...
def custom_create_positive_sample_dualdataset(preproc, data1, data2, positive, baseline=False):
    # Added by me  --> version of create_generic_match_sample where we can send in a pair, one from each dataset
    # only positive examples. don't deal with deduping because it's rare
    # baseline=False means this does the exact opposite -- only gets negative pairs 

    # usercode = True, template_sample = False

    dict_data_users = collections.Counter(data1['usercode'])
    users = list(dict_data_users.keys())
    random.shuffle(users)
    np_users1 = np.array(data1['usercode'])
    np_users2 = np.array(data2['usercode'])
    np_ecgs1 = np.array(data1['ecg'])
    np_ecgs2 = np.array(data2['ecg'])
    
    examples = []

    for ix_u in range(len(users)):
        u = users[ix_u]
        user_index1 = np.where(np_users1 == u)
        if baseline:
            user_index2 = np.where(np_users2 != u)
        else:
            user_index2 = np.where(np_users2 == u)
        
        user_ecgs1 = np_ecgs1[user_index1]
        user_ecgs2 = np_ecgs2[user_index2]

        pairs_list = []
        for i in range(positive):
            first_index = random.randint(0, len(user_ecgs1) - 1)
            second_index = random.randint(0, len(user_ecgs2) - 1)

            # Don't really care if that pair has already been selected; we have so much data it's unlikely
            new_pair = [first_index, second_index]
            pairs_list.append(new_pair)

            first = preproc_reshape_feature_vector(preproc, user_ecgs1[first_index])
            second = preproc_reshape_feature_vector(preproc, user_ecgs2[second_index])

            examples.append({'match_sample': np.concatenate([first, second], axis=1).reshape((-1, 2, 1)),
                            'is_matching': 1,
                            'user': u,
                            'template': first_index,
                            'sample': second_index})
    return examples
...
def add_compile(model, **params):
    ...
    if "eer" in params['metrics']: mets.append(EER())
...
def get_model(**params):
    ...
    elif experiment.find('identification') >= 0: 
        raise ValueError("Hi it's me, they never tested identification")

    else:
        raise ValueError("params['experiment'] in config needs to contain the word verification or autoencoder or identification")
